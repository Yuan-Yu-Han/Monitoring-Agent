        ...
        if hits:
            ...
            for idx, hit in enumerate(hits, 1):
                lines.append(
                    ...
                    f"labels={hit.get('labels','-')}, content={(hit.get('content','') or '')[:140]}"
                )
    if lines:
        return "\n".join(lines)
    return _fallback_case_context(case_memory, query=query, top_k=top_k, days=days, labels=labels)
...
def retrieve_chat_memory(
    *,
    ...
    if lines:
        return "\n".join(lines)
    return _fallback_case_context(case_memory, query=query, top_k=top_k, days=days, labels=None, state="chat")
